...
import { Copy, TrashIcon, Download } from 'lucide-react'
    ...
    return (
        <div className='flex flex-col md:flex-row gap-5 border p-4 bg-gray-900 rounded-lg'>
            ...
            <Link to={`/link/${url?.id}`} className='flex flex-col flex-1'>
                ...
                <span className='text-2xl text-blue-400 font-bold hover:underline cursor-pointer'>https://{DOMAIN}/{url?.custom_url ? url?.custom_url : url?.short_url}</span>
            ...
            <div className="flex gap-2">
                <Button variant='ghost' onClick={() => {
                    navigator.clipboard.writeText(`https://${DOMAIN}/${url?.short_url}`);
                }}>
                    <Copy className='' />
                    Copy Link
                </Button >
                ...
                <Button variant='ghost' onClick={() => { fnDelete().then(() => { fetchUrls(); }) }}>
                    {loadingDelete ? <BeatLoader size={5} color='white' /> : <TrashIcon className='' />}
                    Delete
                </Button>
            </div>
        </div>
